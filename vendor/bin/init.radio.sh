...
#
# Set product codename for QCRIL
#
sku=$(getprop ro.boot.product.hardware.sku)
device=$(getprop ro.product.vendor.device)
if [ "$device" = "alioth" ] && [ "$sku" = "std" ]; then
    setprop ro.vendor.radio.codename "aliothin"
elif [ "$device" = "lmi" ] && [ "$sku" = "pro" ]; then
    setprop ro.vendor.radio.codename "lmipro"
else
    setprop ro.vendor.radio.codename "$device"
fi
